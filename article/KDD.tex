...
solve $n$ different linear equations in one variable. 
Unlike Algorithm~\ref{alg:naive}, this local search function depends
on $\cR$. 
\begin{algorithm}
  ...
  hyperplane $\cR$
  \ENSURE An hyperplane $\cH^{(t)}$ which splits $\Set$
  \STATE $first\_minimum = 1$
  \STATE $second\_minimum = 1$
  \FORALL{$\vec{p}$  in $\Set$}
  \IF{$0<Distance(\vec{p},\cH,\cR)<1$ and 
    $first\_minimum > Distance(\vec{p},\cH,\cR)$}
  \STATE $second\_minimum = first\_minimum$
  \STATE $first\_minimum = Distance(\vec{p},\cH,\cR)$
  \ENDIF
  \ENDFOR
  \STATE $\epsilon= second\_minimum- first\_minimum$
  \STATE $t = first\_minimum+\epsilon/2$
  \RETURN $\cH^{(t)}$
  \end{algorithmic}
\end{algorithm}
Although in a   cryptic way, the geometric intuition of what this
function does is simple. First, consider the two hyperplanes,
\begin{equation}
\label{eq:definition_H_R}
  \cH = \{\vec{x}\in\RR^d : \dotProd{\vec{a}}{\vec{x}}=b\},
  \quad
  \cR = \{\vec{x}\in\RR^d : \dotProd{\vec{r}}{\vec{x}}=s\}.
\end{equation}
We remark that $\cR$ defines the trivial partition, because all points
are in the left side of $\cR$. 

Now, define the following set of hyperplanes,
depending on a parameter $t$,
\begin{equation*}
  \cH^{(t)} = \{\vec{x}\in\RR^d : 
  \dotProd{(t\vec{a}+(1-t)\vec{r})}{\vec{x}}=tb+ (1-t)s\},
  \quad 0\le t \le 1.
\end{equation*}
It is trivial to see that $\cH^{(0)} = \cH$, $\cH^{(1)} = \cR$, for
some value $0\le t\le 1$. Perturbating  $\cH$, in a way that
it splits $\Set$ in the same way, we can suppose that 
$\cH^{(t)}$ contains at most one point in $\Set$ for $0<t<1$. 

In these conditions,
there exists a value $0< t < 1$, such that 
the partition defined by $\cH$ and $\cH^{(t)}$ differ in only one
point $\vec{p}$. This point $\vec{p}$ lays in the right side of $\cH$
and in the left side of $\cH^{(t)}$.  
